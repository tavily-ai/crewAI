...
- `exact_match` (bool, optional): If `True`, only return results containing the exact phrase(s) in quotes within the query. Defaults to `None` (not sent).
...
Use `exact_match=True` to only return results containing the exact phrase(s) in quotes within the query:

```python
# Search for results containing the exact phrase "John Smith"
exact_match_tool = TavilySearchTool(exact_match=True)
result = exact_match_tool.run(query='"John Smith" CEO Acme Corp')
```
