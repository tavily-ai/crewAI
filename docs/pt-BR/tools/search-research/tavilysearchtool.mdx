...
- `exact_match` (bool, optional): Whether to only return results containing the exact phrase(s) inside quotes. Defaults to `None` (not sent).
...
exact_search_tool = TavilySearchTool(exact_match=True)

# Use with quoted phrases in the query
# e.g., query='"John Smith" CEO Acme Corp' will only return results
# containing the exact phrase "John Smith"
```
