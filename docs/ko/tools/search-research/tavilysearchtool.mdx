...
- `search_depth` (Literal["basic", "advanced", "fast", "ultra-fast"], 선택): 검색의 심도입니다. 옵션: `"basic"`, `"advanced"`, `"fast"` (BETA: 높은 관련성과 함께 낮은 지연 시간에 최적화, 1 API 크레딧), `"ultra-fast"` (BETA: 지연 시간을 최우선으로 함, 1 API 크레딧). 기본값은 `"basic"`입니다.
...
- **설정 가능한 깊이**: 기본, 고급, fast (BETA), ultra-fast (BETA) 검색 모드 선택 가능
