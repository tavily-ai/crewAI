...
- `search_depth` (Literal["basic", "advanced", "fast", "ultra-fast"], optional): The depth of the search. Options: `"basic"`, `"advanced"`, `"fast"` (BETA: optimized for low latency with high relevance, 1 API Credit), `"ultra-fast"` (BETA: prioritizes latency above all else, 1 API Credit). Defaults to `"basic"`.
